...
A variant is synthesised only after it passes the self-checking testbench
exhaustively at $8$ bits. Area and timing reports do not distinguish a correct
design from an incomplete one.
\end{column}
...
\item Reconcile the measurement with the prediction.
\end{enumerate}
\end{column}
\end{columns}
...
\begin{frame}{Optimization 3 --- the cost of the separate negation stage}

After Optimization~2 the design is $1180$\um{} larger than the inferred
reference. A cell census localises the difference.
...
mapped to library adder cells.} Both follow from the structure of
\texttt{mux\_and\_shift}, which builds a row in two steps --- a multiplexer picks
...
performing nothing but conditional inversion.

\vspace{4pt}
The indirect cost is the larger of the two. A full adder is itself
XOR-dominated; with an XOR layer feeding the tree, the mapper sees cones that
mix negation and compression and decomposes them into generic gates rather than
matching \texttt{FA\_X1}. The low adder count is a consequence of the XOR layer,
...
\vspace{4pt}
\textbf{The gain is indirect.} Removing the XOR layer does not merely delete
$528$ cells; it lets the mapper match the compressors already present in the
RTL.
...
\begin{column}{0.48\textwidth}
\footnotesize
Exhaustive coverage at $N=8$ is stronger evidence than random coverage at
$N=32$. The properties under test are all \textbf{structural}:

\vspace{4pt}
\begin{itemize}\itemsep2pt
\item the recoding table,
\item the bias cancellation of Optimization~1,
\item the four-term cover of Optimization~3,
\item the column schedule of Optimization~2.
\end{itemize}

\vspace{4pt}
Each is a statement about how the array is constructed, not about particular
operand values, so each fails identically at both widths.

\vspace{6pt}
Each synthesis configuration has a matching testbench configuration, so the
binding verified is the binding synthesised.
\end{column}
\end{columns}
...
\vspace{6pt}
\textbf{The three contributions are unequal, and do not follow the size of the
structural change each makes.}

\vspace{2pt}
Sign-extension elimination removes $29.9\%$ of the array but $5.8\%$ of the
area --- constant propagation removes most of that logic independently of it.

\vspace{2pt}
The Dadda schedule gives a comparable area gain and the largest delay gain,
consistent with a shorter and more uniform reduction depth.

\vspace{2pt}
The fused selector gives the largest area gain, indirectly: not the $528$ XOR
cells it deletes, but the $331$ additional full adders the mapper matches once
they are gone.
\end{column}

\begin{column}{0.44\textwidth}
\footnotesize
\textbf{The general result.}\\[2pt]
The cost of an RTL construct is set as much by what it prevents the tool from
matching as by the gates it implies. One layer of XOR gates between the row
generators and the tree accounts for $\approx 600$\um{}, most of it in the logic
behind the layer rather than in the layer itself.
