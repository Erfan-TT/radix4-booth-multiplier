...
reference, and a minimum area of \textbf{5029\,\textmu m$^2$}\src{R4} against
\textbf{4441\,\textmu m$^2$}\src{R6}. Relative to the unoptimized Wallace
...
A parallel multiplier is one of the few datapath blocks in which the choice of
architecture, rather than the choice of library or the effort level of the
synthesis tool, dominates the result. The objective of this work is to quantify
that dependence: to implement a radix-4 Booth multiplier structurally, apply a
sequence of well-defined architectural optimizations, and measure each one under
an identical synthesis flow.

The design is generic in the operand width $N$. All results reported here are
for $N=32$, signed, with the product taken modulo $2^{2N}$.
...
\item \textbf{Single source tree.} Every variant is an alternative
      ...
      its predecessor only in the component it binds.
\item \textbf{Absolute reference.} A separate design consisting of the single
      statement \texttt{P <= A * B} is synthesised with the same script,
      constraints and library. It provides a fixed point against which the
      structural variants are measured, so that improvements are not reported
      relative to an arbitrary baseline alone.
\item \textbf{Verification precedes measurement.} No variant is synthesised
      before it passes the self-checking testbench. This is not a formality:
      an early variant that skipped this step produced excellent area figures
      precisely because it was missing logic.
\end{itemize}
...
recoding~\cite{booth,macsorley} inspects overlapping triplets of $B$ --- bits
...
The datapath is shown in Figure~\ref{fig:schem} and consists of five stages.
...
The \texttt{mux\_and\_shift} and \texttt{corrector} architectures must be bound
in matching pairs. The \texttt{no\_sign\_extend} row format biases every row by
$+2^{\,2i+N}$ and depends on the matching corrector architecture to cancel it.
The configurations in \texttt{cfg/} enforce the pairing; the language does not.
...
$6346\,\mathrm{\textmu m^2}$\src{R1} to $5975\,\mathrm{\textmu m^2}$\src{R2},
a reduction of $5.8\%$, and the cell count at a $3.0$\,ns constraint falls from
$4639$ to $4455$\src{R7}.

The distance between a $29.9\%$ reduction in partial-product bits and a $5.8\%$
reduction in area is itself informative. The removed bits are constant-valued
sign replications, and constant propagation in \texttt{compile\_ultra} already
eliminates much of the logic that would otherwise process them. What the
optimization removes with certainty, and what constant propagation cannot
remove, is the structural height of the columns. That is what Optimization~2
exploits.
...
independently of the schedule. Half adders fall outside this accounting and are
therefore worth minimising.

\subsection{Wallace and Dadda placement criteria}

Wallace reduction~\cite{wallace} compresses as early as possible: at every
stage, every group of three dots in a column is given a full adder. Dadda
reduction~\cite{dadda} compresses as late as possible: a target height is fixed
...
the bit at position $N$, and the alternating pattern above it. Because this
matrix hard-codes the optimized layout, the Dadda architecture is valid only in
combination with \texttt{mux\_and\_shift(no\_sign\_extend)} or
\texttt{mux\_and\_shift(fused\_selector)}; no configuration pairs it with the
baseline.
...
$C_{\mathrm{in}} + n - 2F - H \leq d_{\mathrm{lev}-1}$. A column must therefore
be sized against a quantity that does not exist in its own layer. Omitting
$C_{\mathrm{in}}$ from the test yields a schedule that is locally plausible and
globally wrong: columns overflow their target height and the array fails to
reach two rows in the allotted number of stages.
...
\texttt{num\_C\_out\_from\_prev\_col} resolves the generate-statement problem
directly: instead of propagating $C_{\mathrm{in}}$ forward, it recomputes it by
counting the compressors of column $k-1$ in the same layer. Two assertions guard
the invariants at elaboration time --- that no column is assigned more than one
half adder, and that the half-adder source positions are unique. A violation
fails elaboration rather than producing a silently mis-wired tree.
...
At relaxed timing the area falls from $5975\,\mathrm{\textmu m^2}$\src{R2} to
$5620\,\mathrm{\textmu m^2}$\src{R3}, a further $5.9\%$, and the minimum
...
After Optimization~2 the design remained $1180\,\mathrm{\textmu m^2}$ larger
than the inferred reference. A cell census of the two netlists at a $3.0$\,ns
constraint\src{R7} localised the difference.
...
The structural design contained $671$ more exclusive-or cells than the inferred
one, and mapped only $117$ of its $435$ scheduled full adders to library adder
cells. The two observations have a common cause. \texttt{mux\_and\_shift} builds
...
negation. That is $N+1$ XOR2 cells per row and $16$ rows, or $528$ cells at
$N=32$, approximately $843\,\mathrm{\textmu m^2}$ performing nothing but
conditional inversion.

The secondary effect is more damaging than the direct cost. A full adder is
itself an exclusive-or-dominated function. With a layer of XOR gates immediately
feeding the reduction tree, the logic cones presented to the technology mapper
mix negation and compression, and the mapper decomposes them into generic gates
instead of recognising the \texttt{FA\_X1} pattern. The low adder-cell count is
a symptom of the XOR layer, not an independent problem.
...
\midrule
Dadda, separate negation & 1230 & 117 & 1405 & 3936 & 5725\,\textmu m$^2$ \\
Dadda, fused selector    &   85 & 448 & 1311 & 2819 & 5125\,\textmu m$^2$ \\
Inferred \texttt{A * B}  &  559 & 268 &  950 & 3035 & 4910\,\textmu m$^2$ \\
\bottomrule
\end{tabular}
...
Removing the XOR layer allowed the mapper to recognise compressors that had been
present in the RTL all along, which is why this optimization produces the
largest area gain of the three despite consisting of a rewrite of a single
architecture.

\subsection{Measured effect}

Area falls from $5620\,\mathrm{\textmu m^2}$\src{R3} to
$5029\,\mathrm{\textmu m^2}$\src{R4}, a reduction of $10.5\%$, and the minimum
...
evaluated independently of the design. Mismatches are counted and reported, so a
failing variant produces a mismatch count rather than a single assertion.

Every variant is verified exhaustively at $N=8$ before being synthesised at
$N=32$. Exhaustive coverage at the smaller width is more valuable than random
coverage at the larger one, because the properties under test --- the recoding
table, the bias cancellation, the completeness of the fused selector's cover and
the column scheduling --- are structural, and their failure modes appear at
$N=8$ exactly as they would at $N=32$.

Two defects were found this way that would not have been found otherwise. In the
first, the $-2A$ row was formed by shifting without complementing, failing $256$
of $1280$ directed cases. In the second, a behavioural variant omitted the Booth
increment entirely and failed $61\,440$ of $65\,536$ exhaustive cases; its
synthesis results were excellent because the missing logic was genuinely absent.
Both defects were invisible in the area and timing reports and immediately
...
netlist; it is simply slower than requested. Discarding such runs would discard
the fastest netlists in the sweep, because the tool produces its most aggressive
structures only when the constraint is tight enough to force them.
...
This was verified not to be an artefact of the area constraint. A controlled
sweep with \texttt{set\_max\_area 0} removed\src{R8} gave identical minimum
achieved periods and area differences below $3\%$, and remained non-monotonic.
The cause is the heuristic nature of the optimizer, not the constraint.
...
\midrule
Wallace, sign-extended (baseline)  & 1.86\,ns & 6346\,\textmu m$^2$ & ---     & \src{R1} \\
\quad $+$ sign-extension elimination & 1.87\,ns & 5975\,\textmu m$^2$ & 19.5\% & \src{R2} \\
\quad $+$ Dadda reduction            & 1.78\,ns & 5620\,\textmu m$^2$ & 38.1\% & \src{R3} \\
\quad \textbf{$+$ fused selector}    & \textbf{1.76\,ns} & \textbf{5029\,\textmu m$^2$} & \textbf{69.1\%} & \src{R4} \\
\midrule
Behavioural Booth (adder chain)    & 2.01\,ns & 6210\,\textmu m$^2$ &  7.1\% & \src{R5} \\
Inferred \texttt{A * B}            & 1.63\,ns & 4441\,\textmu m$^2$ & reference & \src{R6} \\
\bottomrule
\end{tabular}
...
rather than the explicitly instantiated ones. It is included as a measurement of
what inference achieves when given a structurally poor description, and its
timing result is the more informative of its two figures.
...
Table~\ref{tab:census3} gave the cell census at a fixed $3.0$\,ns constraint.
Two observations are worth restating.

First, the area reduction from Optimization~1 ($5.8\%$) is much smaller than the
reduction in partial-product bits ($29.9\%$). Constant propagation removes most
of the logic associated with sign replication before the optimization is
applied, so what the optimization contributes is the reduction in structural
column height, which is realised by Optimization~2.

Second, the three optimizations are not independent. Optimization~3 could not
have produced its measured gain without Optimization~2 in place, because the
compressors that the mapper was then able to recognise are the ones the Dadda
schedule instantiates. Measured in isolation, each optimization would appear
smaller than it does in sequence.
...
variant is smaller by $591\,\mathrm{\textmu m^2}$; at tight constraints it is
faster.

This is an architectural trade-off rather than optimizer noise: the crossing is
reproduced across adjacent constraint points, and it appears in the cell census
as a shift in drive strengths rather than in cell counts.
...
$5029\,\mathrm{\textmu m^2}$ against $4441\,\mathrm{\textmu m^2}$, recovering
$69.1\%$ of the initial area difference.

The individual contributions were unequal in a way not predictable from the
analysis alone. Sign-extension elimination removes $29.9\%$ of the
partial-product bits but only $5.8\%$ of the area, because constant propagation
had already removed most of the associated logic. The Dadda schedule contributes
a comparable area reduction and the largest single improvement in delay. Fusing
the Booth selector contributes the largest area reduction of the three, and does
so indirectly: its measurable effect is not the $528$ exclusive-or cells it
removes but the $331$ additional full adders the technology mapper is then able
to recognise.

The general observation is that the cost of an RTL construct is determined as
much by what it prevents the synthesis tool from recognising as by the gates it
directly implies. In this design a single layer of exclusive-or gates between
the row generators and the reduction tree was worth approximately
$600\,\mathrm{\textmu m^2}$ --- not through its own area, but by concealing the
structure of everything behind it.

Four directions remain open: switching-activity-based power analysis, for which
the removal of $1145$ exclusive-or cells should be more visible than it is in
area; gate-level simulation of each netlist against its SDF; extension of the
sweep below $1.0$\,ns, since every variant reached its fastest netlist at the
tightest constraint attempted; and a Baugh--Wooley~\cite{baughwooley}
implementation as an alternative treatment of the sign problem.
...
testbench instantiates a \emph{component}. Without a second, testbench-level
configuration the default binding rule silently selects the most recently
analysed architecture. Each design configuration therefore has a matching
...
\bibitem{booth} A. D. Booth, ``A Signed Binary Multiplication Technique,''
  \emph{Quarterly Journal of Mechanics and Applied Mathematics}, vol.~4,
  no.~2, pp.~236--240, 1951.
\bibitem{macsorley} O. L. MacSorley, ``High-Speed Arithmetic in Binary
  Computers,'' \emph{Proceedings of the IRE}, vol.~49, no.~1, pp.~67--91, 1961.
\bibitem{wallace} C. S. Wallace, ``A Suggestion for a Fast Multiplier,''
  \emph{IEEE Transactions on Electronic Computers}, vol.~EC-13, no.~1,
  pp.~14--17, 1964.
\bibitem{dadda} L. Dadda, ``Some Schemes for Parallel Multipliers,''
  \emph{Alta Frequenza}, vol.~34, pp.~349--356, 1965.
\bibitem{baughwooley} C. R. Baugh and B. A. Wooley, ``A Two's Complement
  Parallel Array Multiplication Algorithm,'' \emph{IEEE Transactions on
  Computers}, vol.~C-22, no.~12, pp.~1045--1047, 1973.
\bibitem{nangate} \emph{Nangate 45\,nm Open Cell Library}, Nangate Inc.
\bibitem{dc} \emph{Design Compiler User Guide}, W-2024.09-SP2, Synopsys Inc.
\end{thebibliography}
